...
policytext <- data.frame(text = c(cy_tokenized, tsang_tokenized), speaker = c(rep(1, length(cy_tokenized)), rep(0, length(tsang_tokenized))), stringsAsFactor=FALSE)
...
cal_chisq <- function(n, tdm_m, speaker) {
  ...
  cy_r <- (twoxtwo[4] + 0.5) / (twoxtwo[3] + twoxtwo[4] + 1) 
  tsang_r <- (twoxtwo[2] + 0.5) / (twoxtwo[1] + twoxtwo[2] + 1)
  #print(cy_r)
  #print(tsang_r)
  rr <- cy_r / tsang_r
  #print(rr)
...
termschiv <- ldply(1:nrow(tdm_m), cal_chisq, tdm_m = tdm_m, speaker = policytext$speaker)
